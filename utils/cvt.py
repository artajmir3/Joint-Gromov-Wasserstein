...
def centroid_region(vertices, map_2d, memory, threshold=0):
	...
	points_no_zero = points[(map_array != 0), ]
	g0_no_zero = g0[(map_array != 0), ]
	g1_no_zero = g1[(map_array != 0), ]
	g2_no_zero = g2[(map_array != 0), ]
	map_array_no_zero = map_array[(map_array != 0), ]


	# print(points.shape)
	# print(points_no_zero.shape)
	# print(g0_no_zero.shape)
	# print(g1_no_zero.shape)
	# print(g2_no_zero.shape)
	# print(map_array_no_zero.shape)
	# print(map_array.shape)
	# print(np.any(points == 0, axis=1).sum())
	# print(np.any(points == 0, axis=1).shape)
	# print((map_array == 0).shape)
	# print((map_array == 0).sum())
	# print((map_2d[min_x:max_x, min_y:max_y, min_z:max_z] == 0).sum()/(N_x * N_y * N_z))

	if len(points_no_zero) > 0:
		in_array = Delaunay(vertices).find_simplex(points_no_zero) >= 0

		

		A += (in_array*map_array_no_zero).sum()
		C_x += (in_array*map_array_no_zero* g0_no_zero).sum()
		C_y += (in_array*map_array_no_zero* g1_no_zero).sum()
		C_z += (in_array*map_array_no_zero* g2_no_zero).sum()
...
def get_init(map_3d, M, random_seed=None):
	...
	# if random_seed is not None: np.random.seed(seed=random_seed + 1)
