...
def jgw(name, base_path = 'data/embuild/'):
	...
	num_log = 1
	snapshot_idx = [300,301,302,303,304,305,306]
...
#     print(c)
		...
		cur_mu = mu
		if dif[-1] < 1e-2:
			chosen_idxs.append(len(mus) - 1)
			print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!new initial map")
			if costs[-1] < optimal_cost:
				optimal_cost = costs[-1]
				optimal_idx = chosen_idxs[-1]
			cur_mu = generate_random_initial_method3(points_x, points_y, mu_x, mu_y)
	...
	#     if i == 0 and maxi >1e-4:
			...
			out_path = ("data/output/" + name + "chains/JGW_output_%s_%d.pdb")%(chain_ids[k],idx)
			if idx == optimal_idx:
				out_path = ("data/output/" + name + "chains/CHOSEN_JGW_output_%s_%d.pdb")%(chain_ids[k],idx)
